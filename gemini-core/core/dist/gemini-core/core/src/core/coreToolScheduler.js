        ...
        if (allCallsFinalOrScheduled) {
            ...
            callsToExecute.forEach((toolCall) => {
                ...
                scheduledCall.tool
                    ...
                    let resultForDisplay = toolResult;
                    let summary;
                    if (scheduledCall.tool.summarizer) {
                        try {
                            const toolSignal = new AbortController();
                            summary = await scheduledCall.tool.summarizer(toolResult, this.config.getGeminiClient(), toolSignal.signal);
                    ...
                    const response = convertToFunctionResponse(toolName, callId, summary ? [summary] : toolResult.llmContent);

...
import { GEMINI_CONFIG_DIR, setGeminiMdFilename, getCurrentGeminiMdFilename, DEFAULT_CONTEXT_FILENAME, } from '@gemini-core/core/src/tools/memoryTool.js';
...
describe('loadServerHierarchicalMemory', () => {
    ...
    beforeEach(() => {
        ...
        GLOBAL_GEMINI_FILE = path.join(GLOBAL_GEMINI_DIR, getCurrentGeminiMdFilename());
    ...
    it('should load only the global context file if present and others are not (default filename)', async () => {
        const globalDefaultFile = path.join(GLOBAL_GEMINI_DIR, DEFAULT_CONTEXT_FILENAME);
    ...
    it('should load ORIGINAL_GEMINI_MD_FILENAME files by upward traversal from CWD to project root', async () => {
        ...
        const srcGeminiFile = path.join(CWD, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        mockFs.stat.mockImplementation(async (p) => {
            if (p === path.join(PROJECT_ROOT, '.git')) {
                return { isDirectory: () => true };
            }
    ...
    it('should load ORIGINAL_GEMINI_MD_FILENAME files by downward traversal from CWD', async () => {
        const subDir = path.join(CWD, 'subdir');
        const subDirGeminiFile = path.join(subDir, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        const cwdGeminiFile = path.join(CWD, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        mockFs.access.mockImplementation(async (p) => {
        ...
        const { memoryContent, fileCount } = await loadServerHierarchicalMemory(CWD, false, fileService);
        const expectedContent = `--- Context from: ${ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST} ---\nCWD memory\n--- End of Context from: ${ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST} ---\n\n` +
    ...
    it('should load and correctly order global, upward, and downward ORIGINAL_GEMINI_MD_FILENAME files', async () => {
        ...
        const globalFileToUse = path.join(GLOBAL_GEMINI_DIR, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        const projectParentDir = path.dirname(PROJECT_ROOT);
        const projectParentGeminiFile = path.join(projectParentDir, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        const projectRootGeminiFile = path.join(PROJECT_ROOT, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        const cwdGeminiFile = path.join(CWD, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        ...
        const relPathSubDir = path.join('sub', ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
    ...
    it('should ignore specified directories during downward scan', async () => {
        ...
        const ignoredDirGeminiFile = path.join(ignoredDir, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST); // Corrected
        const regularSubDir = path.join(CWD, 'my_code');
        const regularSubDirGeminiFile = path.join(regularSubDir, ORIGINAL_GEMINI_MD_FILENAME_CONST_FOR_TEST);
        mockFs.access.mockImplementation(async (p) => {

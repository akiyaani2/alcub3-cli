...
export class MaestroCommands {
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.on('scan-status', (status) => {
                ...
                if (status.status === 'completed' || status.status === 'remediation_required') {
                    if (progressBar)
                        progressBar.succeed(`Scan completed - Compliance: ${status.complianceScore?.toFixed(1)}%`);
                    socket.disconnect();
                    resolve();
                }
                else if (status.status === 'failed') {
                    if (progressBar)
                        progressBar.fail('Scan failed');
                    socket.disconnect();
    ...
    async performRemediation(scanId, autoApprove) {
    ...
    async displayFinalReport(scanId, format) {
    ...
    async handleGetReport(scanId, options) {
...
//# sourceMappingURL=maestro_enhanced.js.map
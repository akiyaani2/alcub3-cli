 ...
 */
export function createJITCommand() {
    ...
    jit
        ...
        try {
            if (options.approvalId && (options.approve || options.deny)) {
                await processApproval(options.approvalId, options.approve ? true : false, options.comments);
            }
            else {
 ...
 */
async function requestPrivilegeInteractive() {
    ...
    const answers = await inquirer.prompt([
        ...
        },
        {
            ...
            filter: (value) => value ? value.split(',').map((r) => r.trim()) : []
        },
        {
            type: 'confirm',
            name: 'confirm',
 ...
 * Request privilege escalation
 */
async function requestPrivilege(options) {
    ...
    try {
        ...
        const response = await axios.post(`${API_BASE}/jit/request`, {
            role: options.role,
 ...
 */
async function checkStatus(id, watch = false) {
    const spinner = ora('Checking status...').start();
    try {
        const authToken = process.env.ALCUB3_AUTH_TOKEN || 'test-token';
        const response = await axios.get(`${API_BASE}/jit/status/${id}`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
        ...
        if (status.sessionId) {
            // Session status
            table.push(['Session ID', colors.highlight(status.sessionId)], ['User ID', status.userId], ['Role', colors.warning(status.grantedRole)], ['Active', status.isActive ? colors.success('Yes') : colors.error('No')], ['Expires', new Date(status.expiresAt).toLocaleString()], ['Time Remaining', formatTimeRemaining(status.timeRemaining)], ['Risk Score', getRiskScoreDisplay(status.riskScore)]);
        }
        else {
            // Approval status
            table.push(['Request ID', colors.highlight(id)], ['Status', getStatusDisplay(status.status)], ['Created', new Date(status.createdAt).toLocaleString()]);
 ...
 * List active sessions
 */
async function listActiveSessions(userId, watch = false) {
    const spinner = ora('Fetching active sessions...').start();
    try {
        const authToken = process.env.ALCUB3_AUTH_TOKEN || 'test-token';
        const params = userId ? { userId } : {};
        const response = await axios.get(`${API_BASE}/jit/sessions`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            },
            params
        ...
        if (watch) {
            ...
            const socket = io(`${SOCKET_URL}/jit`);
...
async function revokeSession(sessionId, reason) {
    ...
    try {
        const authToken = process.env.ALCUB3_AUTH_TOKEN || 'test-token';
        const response = await axios.delete(`${API_BASE}/jit/session/${sessionId}`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            },
            data: {
 ...
 */
async function reviewPendingApprovals() {
    const spinner = ora('Fetching pending approvals...').start();
    try {
        const authToken = process.env.ALCUB3_AUTH_TOKEN || 'test-token';
        const response = await axios.get(`${API_BASE}/jit/approvals/pending`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
 ...
 * Process an approval
 */
async function processApproval(approvalId, approved, comments) {
    const spinner = ora('Processing approval...').start();
    try {
        const authToken = process.env.ALCUB3_AUTH_TOKEN || 'test-token';
        const response = await axios.post(`${API_BASE}/jit/approve`, {
            approvalId,
            approved,
            comments
        }, {
...
async function showStatistics() {
    ...
    try {
        const authToken = process.env.ALCUB3_AUTH_TOKEN || 'test-token';
        const response = await axios.get(`${API_BASE}/jit/stats`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
        ...
        table.push(['Total Requests', colors.highlight(stats.totalRequests.toString())], ['Auto-Approved', colors.success(stats.autoApproved.toString())], ['Manually Approved', colors.info(stats.manuallyApproved.toString())], ['Denied', colors.error(stats.denied.toString())], ['Revoked', colors.warning(stats.revoked.toString())], ['Active Sessions', colors.highlight(stats.activeSessions.toString())], ['', ''], ['Approval Rate', `${stats.approvalRate.toFixed(1)}%`], ['Auto-Approval Rate', `${stats.autoApprovalRate.toFixed(1)}%`], ['Revocation Rate', `${stats.revocationRate.toFixed(1)}%`]);
        console.log(table.toString());
    }
    catch (error) {
...
async function monitorSession(sessionId) {
    ...
    socket.on('session-update', (status) => {
        ...
        table.push(['Session ID', colors.highlight(status.sessionId)], ['Active', status.isActive ? colors.success('Yes') : colors.error('No')], ['Time Remaining', formatTimeRemaining(status.timeRemaining)], ['Risk Score', getRiskScoreDisplay(status.riskScore)]);
        console.log(table.toString());
...
async function watchApproval(requestId) {
    ...
    socket.on('privilege-request-status', (update) => {
...
function displayApprovalDetails(approval) {
    ...
    table.push(['Approval ID', colors.highlight(approval.approvalId.substring(0, 8) + '...')], ['User', approval.userId], ['Requested Role', colors.warning(approval.requestedRole)], ['Risk Score', getRiskScoreDisplay(approval.riskScore)], ['Requested', formatDistanceToNow(new Date(approval.createdAt), { addSuffix: true })]);
    console.log(table.toString());
    console.log(colors.info('\nJustification:'));

    ...
    try {
        ...
        await authenticationMiddleware(req, res, async () => {
            ...
            if (apiKeyData) {
                ...
                const classificationHeader = String(headers['x-classification-level'] || 'UNCLASSIFIED').toUpperCase();
            ...
            if (body && body.text) {
                ...
                pythonProcess.stdout.on('end', () => {
                    try {
                        ...
                        res.status(500).json({
                            error: 'Internal Server Error: Failed to parse security validation result',
                        });
                    }
                });
                pythonProcess.stderr.on('data', (data) => {
                    ...
                    res.status(500).json({
                        error: 'Internal Server Error: An error occurred during security validation',
                    });
                });
...
export const simpleSecurityMiddleware = (req, res, next) => {
    const { headers } = req;

...
describe('WriteFileTool', () => {
    ...
    beforeEach(() => {
        ...
        mockEnsureCorrectEdit.mockImplementation(async (_currentContent, params, _client, signal) => {
            if (signal?.aborted) {
    ...
    describe('_getCorrectedFileContent', () => {
        ...
        it('should call ensureCorrectEdit for an existing file', async () => {
            ...
            // @ts-expect-error _getCorrectedFileContent is private
            const result = await tool._getCorrectedFileContent(filePath, proposedContent, abortSignal);
            expect(mockEnsureCorrectEdit).toHaveBeenCalledWith(originalContent, {
                old_string: originalContent,
    ...
    describe('execute', () => {
        ...
        it('should return error if params are invalid (path outside root)', async () => {
            ...
            const result = await tool.execute(params, abortSignal);
            expect(result.llmContent).toMatch(/Error: Invalid parameters provided/);
            expect(result.returnDisplay).toMatch(/Error: File path must be within the root directory/);
        });
        ...
        it('should overwrite an existing file with corrected content and return diff', async () => {
            ...
            const result = await tool.execute(params, abortSignal);
            expect(mockEnsureCorrectEdit).toHaveBeenCalledWith(initialContent, {
                old_string: initialContent,
                new_string: proposedContent,
                file_path: filePath,
            }, mockGeminiClientInstance, abortSignal);

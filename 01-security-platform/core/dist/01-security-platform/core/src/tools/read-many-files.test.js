...
describe('ReadManyFilesTool', () => {
    ...
    describe('validateParams', () => {
        ...
        it('should return error if paths array is empty', () => {
            const params = { paths: [] };
            expect(tool.validateParams(params)).toBe('The "paths" parameter is required and must be a non-empty array of strings/glob patterns.');
        });
        ...
        it('should return error if paths array contains an empty string', () => {
            ...
            expect(tool.validateParams(params)).toBe('Each item in "paths" must be a non-empty string/glob pattern.');
        });
        it('should return error if include array contains non-string elements', () => {
            ...
            expect(tool.validateParams(params)).toBe('If provided, "include" must be an array of strings/glob patterns.');
        });
        it('should return error if exclude array contains non-string elements', () => {
            ...
            expect(tool.validateParams(params)).toBe('If provided, "exclude" must be an array of strings/glob patterns.');
        });
    });
    describe('execute', () => {
        ...
        it('should read multiple specified files', async () => {
            ...
            expect(content.some((c) => c.includes('--- file1.txt ---\n\nContent1\n\n'))).toBe(true);
            expect(content.some((c) => c.includes('--- subdir/file2.js ---\n\nContent2\n\n'))).toBe(true);
        ...
        it('should handle glob patterns', async () => {
            ...
            const result = await tool.execute(params, new AbortController().signal);
            const content = result.llmContent;
            expect(content.some((c) => c.includes('--- file.txt ---\n\nText file\n\n'))).toBe(true);
            expect(content.some((c) => c.includes('--- another.txt ---\n\nAnother text\n\n'))).toBe(true);
        ...
        it('should NOT use default excludes if useDefaultExcludes is false', async () => {
            ...
            const result = await tool.execute(params, new AbortController().signal);
            const content = result.llmContent;
            expect(content.some((c) => c.includes('--- node_modules/some-lib/index.js ---\n\nlib code\n\n'))).toBe(true);
            expect(content.some((c) => c.includes('--- src/app.js ---\n\napp code\n\n'))).toBe(true);
            expect(result.returnDisplay).toContain('Successfully read and concatenated content from **2 file(s)**');
        });
        ...
        it('should skip PDF files if not explicitly requested by extension or name', async () => {
            ...
            const result = await tool.execute(params, new AbortController().signal);
            const content = result.llmContent;
            expect(content.some((c) => typeof c === 'string' && c.includes('--- notes.txt ---'))).toBe(true);

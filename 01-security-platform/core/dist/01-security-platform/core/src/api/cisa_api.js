...
export class CISARemediationAPI {
    activeScanProcesses = new Map();
    scanResults = new Map();
    scanStatuses = new Map();
    io;
    pythonPath;
    constructor() {
        this.pythonPath = path.join(process.cwd(), 'security-framework', 'src', 'cisa_remediation_engine.py');
    }
    /**
    ...
    createRouter() {
        ...
        router.post('/scan', async (req, res) => {
            try {
                ...
                const userClassification = req.apiKeyData?.classification ||
        ...
        router.post('/remediate', async (req, res) => {
            try {
                const remediationRequest = req.body;
                // Validate request
                ...
                this.startRemediationProcess(remediationRequest.scanId, scanResult, remediationRequest.autoApprove || false, remediationRequest.modulesToRemediate);
                res.status(202).json({
                    scanId: remediationRequest.scanId,
                    status: 'accepted',
        ...
        router.get('/report/:scanId', async (req, res) => {
            try {
                ...
                const filteredResult = this.filterResultsByClassification(scanResult, userClassification);
                if (format === 'summary') {
     ...
     * Start scan process
     */
    startScanProcess(scanId, scanRequest, classification) {
        const args = [
            this.pythonPath,
     ...
     * Start remediation process
     */
    startRemediationProcess(scanId, scanResult, autoApprove, modulesToRemediate) {
     ...
     */
    updateScanProgress(scanId, progressMessage) {
        const status = this.scanStatuses.get(scanId);
        if (status) {
     ...
     * Update scan status
     */
    updateScanStatus(scanId, newStatus) {
        const status = this.scanStatuses.get(scanId);
        if (status) {
     ...
     */
    broadcastScanUpdate(scanId, status) {
        if (this.io) {
     ...
     */
    filterResultsByClassification(result, userClassification) {
...
//# sourceMappingURL=cisa_api.js.map
...
maestroRouter.get('/status', async (_req, res) => {
    try {
...
maestroRouter.get('/health', (_req, res) => {
    res.json({
        status: 'healthy',
        version: '1.0.0',
        timestamp: Date.now()
...
maestroRouter.get('/metrics', async (_req, res) => {
    try {
        const serviceStatus = await maestroClient.getServiceStatus();
...
maestroRouter.post('/validate', async (req, res) => {
    try {
        // Security validation is handled by middleware
        // If we reach here, validation passed
        const validationResult = req.securityValidation;
        res.json({
...
maestroRouter.get('/admin/keys', (_req, res) => {
    try {
        const apiKeys = authManager.listApiKeys();
        res.json({
            keys: apiKeys,
...
maestroRouter.post('/admin/keys', async (req, res) => {
    try {

...
class ConfigurationDriftCLI {
  ...
  async createBaseline(options: any): Promise<void> {
  ...
  async listBaselines(options: any): Promise<void> {
    const spinner = ora('Retrieving configuration baselines...').start();

    try {
      const queryParams = new URLSearchParams();
      if (options.type) queryParams.append('baseline_type', options.type);
      if (options.classification) queryParams.append('classification_level', options.classification);
      ...
      result.baselines.forEach((baseline: any) => {
        table.addRow({
  ...
  async detectDrift(options: any): Promise<void> {
    ...
    try {
      ...
      if (result.detection_result.anomaly_detected) {
        ...
        if (result.detection_result.drift_events.length > 0) {
          ...
          result.detection_result.drift_events.forEach((event: DriftEvent) => {
            table.addRow({
        ...
        if (result.detection_result.recommendations.length > 0) {
          console.log(chalk.yellow('\nRecommendations:'));
          result.detection_result.recommendations.forEach((rec: string, index: number) => {
  ...
  async getMonitoringStatus(options: any): Promise<void> {
  ...
  async stopMonitoring(options: any): Promise<void> {
  ...
  async createRemediationPlan(options: any): Promise<void> {
  ...
  async executeRemediation(options: any): Promise<void> {
    ...
    try {
      ...
      if (result.execution_result.error_messages.length > 0) {
        console.log(chalk.red(`\n❌ Errors:`));
        result.execution_result.error_messages.forEach((error: string, index: number) => {
  ...
  async approveRemediation(options: any): Promise<void> {
  ...
  async listPendingApprovals(options: any): Promise<void> {
  ...
  async getStatistics(options: any): Promise<void> {
    const spinner = ora('Retrieving configuration drift statistics...').start();

    try {
      const queryParams = new URLSearchParams();
      if (options.timeRange) queryParams.append('time_range', options.timeRange);
...
export function createConfigurationDriftCommand(): Command {
